...
entrada_hash_t tabela[TAM_TOTAL];
...
{
    FILE *f = fopen("matricula_hash_reserva.txt", "w");
...
/* Soma dos codigos ASCII do nome */
...
/* Insere restaurante na tabela hash.
   Retorna a posicao inserida ou -1 se nao foi possivel. */
int inserir_hash(restaurante_t r)
{
    int pos = hash(r.nome);

    if (tabela[pos].ocupado == 0)
    {
        tabela[pos].dado   = r;
    ...
    /* Colisao: procura na area de reserva */
    int res = TAM_TABELA; /* começa em 31 */
    while (res < TAM_TOTAL)
    {
        if (tabela[res].ocupado == 0)
        {
            tabela[res].dado   = r;
            tabela[res].ocupado = 1;
            return res;
        }
        res++;
    }

    /* Reserva cheia: nao insere */
    printf("%s\n", r.nome);
    return -1;
}

/* Busca restaurante pelo nome.
   Retorna a posicao (0-49) ou -1 se nao encontrado.
   Conta comparacoes. */
int buscar_hash(const char *nome)
{
    ...
    comparacoes++;
    if (tabela[pos].ocupado == 1 && strcmp(tabela[pos].dado.nome, nome) == 0)
    {
        return pos;
    }

    /* Busca na area de reserva */
    int res = TAM_TABELA;
    while (res < TAM_TOTAL)
    {
        comparacoes++;
        if (tabela[res].ocupado == 1 && strcmp(tabela[res].dado.nome, nome) == 0)
        {
            return res;
        }
        res++;
...
int main()
{
    ...
    while (scanf(" %[^\n]", nome_busca) == 1 && strcmp(nome_busca, "FIM") != 0)
    {
        ...
        else
        {
            ...
            formatar_restaurante(resultado + strlen(resultado), tabela[pos].dado);
            printf("%s\n", resultado);
        }
    }
    fim_clock = clock();
