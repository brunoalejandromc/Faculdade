...
class No
{
    ...
    public Restaurante getRegistro()
    {
        return registro;
    }
    public void setRegistro(Restaurante registro)
    {
...
class ColecaoRestaurantes
{
    ...
    private static String removerBarraR(String s) 
{
    int len = s.length();
...
class Arvore
{
    ...
    private int comparacoesInsercao = 0;
    private int comparacoesPesquisa = 0;
    ...
    private void atualizarAltura(No no)
    {
        ...
        alturaEsq = altura(no.getEsq());
        alturaDir = altura(no.getDir());
    ...
    private No balancear(No no)
    {
        ...
        if(fator > 1 && fatorBalanceamento(no.getEsq()) >= 0)
        {
        ...
        if(fator > 1 && fatorBalanceamento(no.getEsq()) < 0)
        {
            no.setEsq(rotacionarEsquerda(no.getEsq()));
            return rotacionarDireita(no);
        }

        if(fator < -1 && fatorBalanceamento(no.getDir()) <= 0)
        {
        ...
        if(fator < -1 && fatorBalanceamento(no.getDir()) > 0)
        {
            no.setDir(rotacionarDireita(no.getDir()));
            return rotacionarEsquerda(no);
        }
    ...
    private No inserirNo(No no, Restaurante r)
    {
        int cmp = 0;
        ...
        cmp = compararNomes(r.getNome(), no.getRegistro().getNome());

        if (cmp < 0)
        {
            no.setEsq(inserirNo(no.getEsq(), r));
        }
        else if (cmp > 0)
        {
            no.setDir(inserirNo(no.getDir(), r));
        }
        else
        {
            return no;
    ...
    public void pesquisar(String nome)
    {
        ...
        while(atual != null && !encontrado)
        {
            if(profundidade == 0)
            {
                caminho[profundidade++] = "raiz";
            }

            comparacoesPesquisa++;

            cmp = compararNomes(nome, atual.getRegistro().getNome());
            ...
            {
                caminho[profundidade++] = "esq";
                atual = atual.getEsq();
            }
            else
            {
                caminho[profundidade++] = "dir";
                atual = atual.getDir();
            }
        }
    ...
    private void emOrdem(No no)
    {
        ...
        emOrdem(no.getEsq());
        System.out.println(no.getRegistro().formatar());
        emOrdem(no.getDir());
    }
...
class Ex01
{
    public static void main(String[] args) throws Exception
    {
        ...
        FileWriter fw = new FileWriter("832656_arvore_avl.txt");
        fw.write("832656\t" + arvore.getComparacoesPesquisa() + "\t" + String.format("%.2f", tempo));
        fw.close();
